...
An empirical, mathematical combat simulator and analytical tier list engine for the mobile game *Arknights*. The project replaces subjective community rankings with deterministic, objective combat simulations over 300-second (5-minute) standard battle encounters. (Up to date to 2026-09-14 Global)
...
- **The static snapshot page — browsable, but no functionality.** Published to GitHub Pages by [`.github/workflows/publish-static-snapshot.yml`](.github/workflows/publish-static-snapshot.yml), which rebuilds automatically whenever `data/Automated_Operators.json`/`Automated_Enemies.json` change (or on demand). It's the same cached Tier List data (`data/cache/<hash>/operator_tierlist.json`/`enemy_tierlist.json`, 1.3.0) rendered by a small standalone page (`static_site/index.html`) — browse both Tier Lists, every category, tier-grouped, with portraits, no server behind it at all. No Team Builder, no recalculation, no editing — once GitHub Pages is enabled for this repo (Settings → Pages → Source: **GitHub Actions**, one-time setup), it lives at `https://<your-username>.github.io/<repo-name>/`.
