...
- **Minutes-remaining counter**: the workflow's first step prints how many GitHub Actions minutes are left this month (only meaningful for a private repo — public repos get unlimited free minutes, but it's harmless to show either way), if a `BILLING_PAT` repository secret is set (a classic PAT, no special scopes needed to read your own billing). Skips cleanly with a pointer to the README if the secret isn't there.
- Documented in `README.md`: which URL is "just docs, no functionality" (the GitHub repo page itself, always available) vs. "the real live app" (the temporary tunnel URL printed once the workflow starts), and that `workflow_dispatch` already requires write access to the repo — a stranger can't trigger this even on a public repo.
